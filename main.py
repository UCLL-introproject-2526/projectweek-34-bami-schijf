...
async def main():
    ...
    wave_overlay = None
    wave_overlay_timer = 0
    WAVE_OVERLAY_DURATION = 120  # frames (~2 seconds at 60 FPS)
    ...
    while running:
        ...
        if enemies == list() and current_wave <= 4:
            ...
            wave_overlay = wave_images[next_wave_number]
            wave_overlay_timer = WAVE_OVERLAY_DURATION
        ...
        def reset_game():
            nonlocal player, enemies, hearts, currentwave, kills_this_wave
            nonlocal total_enemies_in_wave, text, game_start, invincible, stunned
            nonlocal wave_overlay, wave_overlay_timer, foo
            nonlocal music_on
            ...
            currentwave = 1
            kills_this_wave = 0
            total_enemies_in_wave = sum(allenemywaves[1])
            enemies = startnewave(currentwave, hearts)

            # UI / flow state
            text = Text()          # ✅ tutorial screen back
            game_start = False     # ✅ enemies don't move
            invincible = False
            stunned = False
            ...
            # Flags
            foo = True             # ✅ allow game-over sound again
            cangonextwave = True

            # Timer reset
            player.alive_start = None
            player.alive_end = None
        ...
        for event in pygame.event.get():
            ...
            if event.type in (pygame.MOUSEBUTTONDOWN, pygame.KEYDOWN):
                if not music_started:
                    # only start playback on first user interaction if music isn't already playing
                    try:
                        if not pygame.mixer.music.get_busy():
                            pygame.mixer.music.play(-1)
                    except Exception:
                        # some environments may not support get_busy(); attempt to play anyway
                        try:
                            pygame.mixer.music.play(-1)
                        except Exception:
                            pass
        ...
        if wave_overlay and wave_overlay_timer > 0:
            overlay_rect = wave_overlay.get_rect(
                center=(screen_size[0] // 2, screen_size[1] // 2 - 50)
            )
            screen.blit(wave_overlay, overlay_rect)
            wave_overlay_timer -= 1
        else:
            wave_overlay = None
